...
export default class GraphDataStructureParams extends ComplexCanvasAnimationParams<GraphDataStructureParamsType> {
    ...
    protected getIncludedAnimationParamsByParameter (object: GraphDataStructureParamsType): Map<string, SimpleCanvasAnimationParams> {
        ...
        vertexIdMap.forEach((v, k) => {
            ...
            dagreGraph.setNode(k, {
                width: vertexDiameter,
                height: vertexDiameter
            })
        })
        edgeIdMap.forEach((v) => dagreGraph.setEdge(v.sourceId, v.targetId))
        ...
        dagreGraph.edges().forEach((e) => {
            ...
            const edgeStartPoint = this.calculateEdgePoint(vVertexDiameter, addPoints(origin, vNode), startVertexAngle)
            const edgeEndPoint = this.calculateEdgePoint(wVertexDiameter, addPoints(origin, wNode), endVertexAngle)
        ...
        pointedToItselfVertexIdEdgeMap.forEach((edge, vertexId) => {
            ...
            new ArrowCanvasAnimationParams({
                object: {
                    ...this.calculateArrowParams(animationStyle, edge, edgeStyle),
                    origin: addPoints(object.origin, dagreNode, points[0]),
                    endPoint: addPoints(object.origin, dagreNode, points[3]),
                    bezierParams: {
                        point2: addPoints(object.origin, dagreNode, points[1]),
                        point3: addPoints(object.origin, dagreNode, points[2])
                    }
                }
            }, this.p5, animationStyle).getIncludedAnimationParams().forEach((value, key) => {
